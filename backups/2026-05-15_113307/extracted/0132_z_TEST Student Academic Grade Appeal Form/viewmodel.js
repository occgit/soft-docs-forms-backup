    ...
    */
    ...
    vm.setDefaults = function setDefaults(source, inputValues) {
    	        ...
    	        vm.ethosGUID(personResults[0].id);
        	        ...
        	        integration.all('Web_Ethos_Section_Registration_by_Registrant',{
                        ...
                        registrationResults.forEach(function(registrationSection, index){
                            ...
                            if (registrationSection.status.registrationStatus == 'registered' && registrationSection.involvement.endOn >= concatDate){
                                p.push(                            
        ...
        vm.locationGUID.subscribe(function (newValue) {
            integration.all('Web_Ethos_Get_Campus_Sites_by_GUID',{
        ...
        vm.sectionGUID.subscribe(function (newValue) {
            integration.all('Web_Ethos_Get_Student_Transcript_Grades_by_STC_GUID', {
        ...
        vm.sectionGUID.subscribe(function (newValue) {
            integration.all('Web_Ethos_Get_Instructors_by_Section_GUID', {
                 ...
                 instructorData.forEach(function (Results) {
                     if (Results.instructorRole == 'primary') {
                         ...
                         integration.all('Web_Ethos_Get_Person_by_GUID', {
                            ...
                            vm.instructorEmail(getPrimaryEmail(instructorInformation.emails));
                        });
        ...
        vm.gradeGUID.subscribe(function (newValue) {
            integration.first('Web_Ethos_Get_Grade_Definitions_by_ID', {
                ...
                console.log("grade val", gradeVal);
           
                if(gradeVal === 'N'){
                    notify('error','You cannot use this form to remove an N grade');
                }
        ...
        if(pkg.isAtStep('ConditionalActorInstructor')){
            $('#instructorSubmitChangeGrade').prop('required', true);
        } else if(pkg.isAtStep('ConditionalActorDepartmentChair')){
             $('#deptChairSubmitChangeGrade').prop('required', true);
        }else if(pkg.isAtStep('ConditionalActorAdministrativeReviewer')) {
             $('#deanSubmitChangeGrade').prop('required', true);
        }

        //Conditionally Shpw/Hide rows based upon WF steps
        if (pkg.stepCode === 'ConditionalActorInstructor' || pkg.stepCode === 'ConditionalActorOriginatorEscalateToDeptChair' || pkg.stepCode === 'ConditionalActorDepartmentChair' || pkg.stepCode === 'ConditionalActorOriginatorEscalateToAdminApprover' || pkg.stepCode === 'ConditionalActorAdministrativeReviewer' || pkg.stepCode === 'ConditionalActorAdministrativeReviewer' || pkg.stepCode === 'ApprovalEndStep' || pkg.stepCode === 'NotEscalatedEndStep' || pkg.stepCode === 'DeniedEndStep'  || user.isInActivity) { 
        ...
        if (pkg.stepCode === 'ConditionalActorOriginatorEscalateToDeptChair' || pkg.stepCode === 'ConditionalActorDepartmentChair' || pkg.stepCode === 'ConditionalActorOriginatorEscalateToAdminApprover' || pkg.stepCode === 'ConditionalActorAdministrativeReviewer' || pkg.stepCode === 'ConditionalActorAdministrativeReviewer' || pkg.stepCode === 'ApprovalEndStep' || pkg.stepCode === 'NotEscalatedEndStep' || pkg.stepCode === 'DeniedEndStep'  || user.isInActivity) { 
        ...
        if (pkg.stepCode === 'ConditionalActorDepartmentChair' || pkg.stepCode === 'ConditionalActorOriginatorEscalateToAdminApprover' || pkg.stepCode === 'ConditionalActorAdministrativeReviewer' || pkg.stepCode === 'ConditionalActorAdministrativeReviewer' || pkg.stepCode === 'ApprovalEndStep' || pkg.stepCode === 'NotEscalatedEndStep' || pkg.stepCode === 'DeniedEndStep'  || user.isInActivity) { 
        ...
        if (pkg.stepCode === 'ConditionalActorOriginatorEscalateToAdminApprover' || pkg.stepCode === 'ConditionalActorAdministrativeReviewer' || pkg.stepCode === 'ConditionalActorAdministrativeReviewer' || pkg.stepCode === 'ApprovalEndStep' || pkg.stepCode === 'NotEscalatedEndStep' || pkg.stepCode === 'DeniedEndStep'  || user.isInActivity) { 
        ...
        if (pkg.stepCode === 'ConditionalActorAdministrativeReviewer' || pkg.stepCode === 'ConditionalActorAdministrativeReviewer' || pkg.stepCode === 'ApprovalEndStep' || pkg.stepCode === 'NotEscalatedEndStep' || pkg.stepCode === 'DeniedEndStep'  || user.isInActivity) { 
