define([
  ...
  vm.setDefaults = function setDefaults(source, inputValues) {
    ...
    if (user.isInLibrary) {
      ...
      source.Etrieve_Security_Get_Group_Info.forEach(function (rowData, index) {
        //console.log(rowData)
        // add new empty row
        vm.groupRow.push({});

        // populate new rows with the data from the integration
        vm.groupRow()[index].groupName(rowData.GroupName);
        vm.groupRow()[index].principalID(rowData.PrincipalID);
      });
    } else {
