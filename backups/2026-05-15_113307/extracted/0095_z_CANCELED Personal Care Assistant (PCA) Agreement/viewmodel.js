define(['jquery',
    ...
    //Hide Agency Contact Signature Section
    $('.agencyContact').hide();    
    //Hide Personal Care Assistant Section
    $('.pca').hide();
    ...
    */
    ...
    vm.setDefaults = function setDefaults(source, inputValues) {
        ...
        if (pkg.isAtStep('ESignAgencyContact') || pkg.isAtStep('ESignAgencyPCA') || pkg.isAtStep('End')) {
            $('.agencyContact').show();            
        }

        if (pkg.isAtStep('ESignAgencyPCA') || pkg.isAtStep('ESignIndividualPCA') || pkg.isAtStep('End')) {
            $('.pca').show();
        }
        ...
        vm.agencyContactSign.subscribe(function(newValue) {
            if (newValue) {
               vm.agencyContactSignDate(Extenders.utils.formatDate(new Date())); 
            } else {
                vm.agencyContactSignDate('');
            }
        });
        
        vm.pcaSign.subscribe(function(newValue) {
            if (newValue) {
               vm.pcaSignDate(Extenders.utils.formatDate(new Date())); 
            } else {
                vm.pcaSignDate('');
            }
        });

        vm.pcaEmailEnter.subscribe(function(newValue){
            if (newValue) {
                vm.agencyPCAEmail(newValue);               
            } else {
                vm.agencyPCAEmail('');
            }
        });
    ...
    vm.afterLoad = function afterLoad() {
        ...
        ko.computed(function(){
            if (user.isInLibrary  || user.isInDrafts) {
                //validate agency contact email
                if (vm.contactEmail() && vm.contactEmailVerify()){
                    if (vm.contactEmail() !== vm.contactEmailVerify()) {
                        notify('error', 'The contact emails must match!');
                        vm.contactEmailVerify('');
                    } else {
        ...
        ko.computed(function(){
            if (vm.type() == 'Individual'  && (user.isInLibrary  || user.isInDrafts)) {
                if (vm.pcaEmail() !== vm.pcaEmailVerify()) {
                    notify('error', 'The contact emails must match!');
                    vm.pcaEmailVerify('');
                } else {
                    if (vm.pcaEmail() && vm.pcaEmailVerify())
