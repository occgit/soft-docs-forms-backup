    ...
    */
	...
	vm.afterLoad = function afterLoad() {
		...
		ko.computed(function () {
			...
			if (vm.englishOrCommunications()) checkedCount++;
			if (vm.mathOrScience()) checkedCount++;
			if (vm.socialScience()) checkedCount++;
			if (vm.fineArtsOrHumanities()) checkedCount++;
			if (vm.writtenCommunication()) checkedCount++;

			// Require at least one checkbox to be checked
			if (checkedCount === 0) {
				// Set all checkboxes to required
				$("#englishOrCommunications").prop("required", true);
				$("#mathOrScience").prop("required", true);
				$("#socialScience").prop("required", true);
				$("#fineArtsOrHumanities").prop("required", true);
				$("#writtenCommunication").prop("required", true);
			} else {
				// At least one checkbox is checked, so set all checkboxes to not required
				$("#englishOrCommunications").prop("required", false);
				$("#mathOrScience").prop("required", false);
				$("#socialScience").prop("required", false);
				$("#fineArtsOrHumanities").prop("required", false);
				$("#writtenCommunication").prop("required", false);
			}
		});

		ko.computed(function () {
			// Count how many checkboxes are checked
			var checkedCountTwo = 0;

			if (vm.englishOrCommunications()) checkedCountTwo++;
			if (vm.mathOrScience()) checkedCountTwo++;
			if (vm.socialScience()) checkedCountTwo++;
			if (vm.fineArtsOrHumanities()) checkedCountTwo++;
			if (vm.writtenCommunication()) checkedCountTwo++;

			// Get all checkboxes
			var checkboxes = [
				"englishOrCommunications",
				"mathOrScience",
				"socialScience",
				"fineArtsOrHumanities",
				"writtenCommunication"
			];

			if (checkedCountTwo > 1) {
				// Prevent further checkbox selections
		...
		//start ko.computed for MTA Checkboxes
		ko.computed(function () {
			// Count how many checkboxes are checked
			var checkedCount = 0;

			if (vm.mtaEnglishComposition()) checkedCount++;
			if (vm.mtaComms()) checkedCount++;
			if (vm.mtaMath()) checkedCount++;
			if (vm.mtaSocialSciences()) checkedCount++;
			if (vm.mtaFineArtsHumanities()) checkedCount++;
			if (vm.mtaNaturalScience()) checkedCount++;

			// Require at least one checkbox to be checked
			if (checkedCount === 0) {
				// Set all checkboxes to required
				$("#mtaEnglishComposition").prop("required", true);
				$("#mtaComms").prop("required", true);
				$("#mtaMath").prop("required", true);
				$("#mtaSocialSciences").prop("required", true);
				$("#mtaFineArtsHumanities").prop("required", true);
				$("#mtaNaturalScience").prop("required", true);
			} else {
				// At least one checkbox is checked, so set all checkboxes to not required
				$("#mtaEnglishComposition").prop("required", false);
				$("#mtaComms").prop("required", false);
				$("#mtaMath").prop("required", false);
				$("#mtaSocialSciences").prop("required", false);
				$("#mtaFineArtsHumanities").prop("required", false);
				$("#mtaNaturalScience").prop("required", false);
			}
		});
		...
		ko.computed(function () {
			...
			if (vm.mtaEnglishComposition()) checkedCountTwo++;
			if (vm.mtaComms()) checkedCountTwo++;
			if (vm.mtaMath()) checkedCountTwo++;
			if (vm.mtaSocialSciences()) checkedCountTwo++;
			if (vm.mtaFineArtsHumanities()) checkedCountTwo++;
			if (vm.mtaNaturalScience()) checkedCountTwo++;

			// Get all checkboxes
			var checkboxes = [
				"mtaEnglishComposition",
				"mtaComms",
				"mtaMath",
				"mtaSocialSciences",
				"mtaFineArtsHumanities",
				"mtaNaturalScience"
			];
	...
	vm.afterRequired = function afterRequired(form) {
		...
		if (
			vm.genEdDistListChoice() === "Yes" &&
			vm.michiganTransfer() === "Yes"
		) {
			if (form.attachmentCount < 2) {
				notify("error", "You must attach Form OCC 63K and Form OCC63N");
				return false;
			}
		} else if (
			vm.genEdDistListChoice() === "Yes" &&
			...
			notify("error", "You must attach Form OCC 63K");
			return false;
		} else if (
			vm.michiganTransfer() === "Yes" &&
			form.attachmentCount < 1
		) {
			notify("error", "You must attach Form OCC 63N");
			return false;
		}

		if (form.attachmentCount < 1) {
			notify("error", "Please attach your copy of the Ballot Results");
			return false;
		} else {
			return true;
		}
