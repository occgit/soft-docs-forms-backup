define([
  ...
  function disableElements() {
    ...
    autoCompleteUtils.makeReadOnly("supervisorSearch");
  ...
  vm.onSubmit = function onSubmit(form) {
    ...
    //     return false;
    // }
    //Removed per department request 2026-04-28
  ...
  vm.beforeRequired = function beforeRequired(form) {
    ...
    if (isEquipFur.checked) {
      for (var i = 0; i < vm.itemInfoRow().length; i++) {
        if (
          !vm.itemInfoRow()[i].itemDescription() &&
          !vm.itemInfoRow()[i].itemCurrentLocation() &&
          !vm.itemInfoRow()[i].itemQuantity() &&
          !vm.itemInfoRow()[i].ddItemActionType()
        ) {
          notify(
            "error",
            "You must provide the information for at least one item for transfer or disposal to submit this form.",
          );
          return false;
        }

        if (
          vm.itemInfoRow()[i].ddItemCondition() === "NonOperational" &&
          !vm.itemInfoRow()[i].itemNonOperationalCondition()
        ) {
          notify(
            "error",
            "Description of why item is non-operational is required.",
          );
          return false;
        } else if (
          vm.itemInfoRow()[i].ddItemCondition() === "Other" &&
          !vm.itemInfoRow()[i].itemOtherCondition()
        ) {
        ...
        if (
          isCheckedOver5k &&
          (!vm.itemInfoRow()[i].mfgSerialNumber() ||
            !vm.itemInfoRow()[i].occAssetTagNumber())
        ) {
        ...
        const val =
          typeof row.mfgSerialNumber === "function"
            ? row.mfgSerialNumber()
            : row.mfgSerialNumber;
    ...
    if (isVehicle.checked) {
      for (var i = 0; i < vm.vehicleInfoRow().length; i++) {
        if (
          !vm.vehicleInfoRow()[i].vehicleDescription() &&
          !vm.vehicleInfoRow()[i].vehicleLocation() &&
          !vm.vehicleInfoRow()[i].vehicleVIN()
        ) {
          notify(
            "error",
            "You must provide the information for at least one vehicle for transfer or disposal to submit this form.",
          );
          return false;
        }

        if (
          vm.vehicleInfoRow()[i].ddVehicleCondition() === "Non-Operational" &&
          !vm.vehicleInfoRow()[i].vehicleIssueCondition()
        ) {
          notify(
            "error",
            "Description of why item is non-operational is required.",
          ...
          vm.vehicleInfoRow()[i].ddVehicleCondition() === "Other" &&
          !vm.vehicleInfoRow()[i].vehicleOtherCondition()
        ) {
          notify("error", "Description of why item is Other is required.");
          return false;
        }
