define([
  ...
  vm.setDefaults = function setDefaults(source, inputValues) {
    ...
    integration.all("OIE_Get_Deans").then(function (deanobj) {
      ...
      const uniqueDeans = Array.from(uniqueDeansMap.values()).sort((a, b) => {
      ...
      let isDeanEmailMatch = vm.uniqueDeanList().some(function (dean) {
        return (
          dean.Dean_Email && dean.Dean_Email.trim().toLowerCase() === userEmail
        );
      });
    ...
    vm.academicDeanSignature.subscribe(function (newValue) {
      if (newValue) {
        vm.academicDeanSignatureDate(Extenders.utils.formatDate(new Date()));
      } else {
