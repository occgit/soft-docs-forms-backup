define([
  ...
  //hide the company signature section until compnay workflow step
  //$('.companySection').hide();
  ...
  vm.setDefaults = function setDefaults(source, inputValues) {
    ...
    if (pkg.isAtStep("ESignPlacementSite") || pkg.isAtStep("End")) {
      $(".companySection").show();

      $("#contactForPlacementSite").prop("disabled", true);
      $("#contactTitle").prop("disabled", true);
      $("#placementSiteContactTitle").prop("disabled", true);
      $("#placementSiteEmailValidate").prop("disabled", true);
      $("#singleLocation").prop("disabled", true);
      $("#singleLocationAddress").prop("disabled", true);
      $("#singleLocationCity").prop("disabled", true);
      autoCompleteUtils.makeReadOnly("singleLocationState");
      $("#singleLocationZipCode").prop("disabled", true);

      //dynamic list
      vm.quickInputsRow170().forEach(function (dynamicListRow, index) {
        dynamicListRow.multipleLocationsLocation.disabled = true;
        dynamicListRow.multipleLocationsCity.disabled = true;
        dynamicListRow.multipleLocationsState.disabled = true;
        dynamicListRow.multipleLocationsZip.disabled = true;

        $(".btn-delete-item").prop("disabled", true);
      });

      $(".addLocationBtn").prop("disabled", true).attr("aria-disabled", "true");

      $("#allLocations").prop("disabled", true);
      $("#programCoordinatorSignature").prop("disabled", true);
    }

    if (pkg.isAtStep("ESignPlacementSite")) {
      $(".contractInfo").hide();
    }
    ...
    vm.contactTitle.subscribe(function (newValue) {
      if (newValue) {
        vm.coordinatorTitleForSignature(newValue);
      } else {
        vm.coordinatorTitleForSignature("");
      }
    });

    vm.programCoordinatorSignature.subscribe(function (newValue) {
      if (newValue) {
        vm.programCoordinatorSignatureDate(
    ...
    vm.companyContactSignature.subscribe(function (newValue) {
      if (newValue) {
        vm.companyContactSignatureDate(Extenders.utils.formatDate(new Date()));
      } else {
        vm.companyContactSignatureDate("");
      }
    });

    vm.companyName.subscribe(function (newValue) {
      if (newValue) {
        vm.companyNameForSignature(newValue);
      } else {
        vm.companyNameForSignature("");
      }
    });

    vm.placementSiteContactTitle.subscribe(function (newValue) {
      if (newValue) {
        vm.companyContactTitleForSignature(newValue);
      } else {
        vm.companyContactTitleForSignature("");
      }
    });
  ...
  vm.afterLoad = function afterLoad() {
    ...
    ko.computed(function () {
      if (user.isInLibrary || user.isInDrafts) {
        if (vm.placementSiteEmail() && vm.placementSiteEmailValidate()) {
          if (vm.placementSiteEmail() !== vm.placementSiteEmailValidate()) {
            notify(
              "error",
              "Emails for the placement site contact must match!",
            );
            vm.placementSiteEmailValidate("");
          } else {
            notify("success", "Emails match!");
          }
        }
      }
    }).extend({ throttle: 500 });
