define([
  ...
  vm.setDefaults = function setDefaults(source, inputValues) {
    ...
    // vm.parentInfo.subscribe(newVal => {
    //     debugger
    // })
  ...
  vm.afterLoad = function afterLoad() {
    ...
    ko.computed(function () {
      vm.parentInfo.forEach((row, index) => {
        if (row.parentGuardianName() && row.parentGuardianEmail()) {
          let name = `parentGuardianName${index + 1}`;
          let email = `parentGuardianEmail${index + 1}`;

          vm[name](row.parentGuardianName());
          vm[email](row.parentGuardianEmail());
        }
      });
    });

    ko.computed(function () {
      vm.authorizedPickups.forEach((row, index) => {
        if (row.authorizedPickupName() && row.authorizedPickupPhone()) {
          let name = `authPickupName${index + 1}`;
          let phone = `auhtPickupPhone${index + 1}`;

          vm[name](row.authorizedPickupName());
          vm[phone](row.authorizedPickupPhone());
        }
      });
    });
