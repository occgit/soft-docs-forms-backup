...
define([
    ...
    */
	...
	vm.setDefaults = function setDefaults(source, inputValues) {
		...
		vm.formName(
			"ADS-72 Prerequisite/Assessment Exemption - Employee Initiated"
		);

		vm.occRole.subscribe(function (roleValue) {
			roleRowShow(roleValue);
		});

		vm.getStudentByErpId = function () {
			try {
				if (vm.colleagueID()) {
					integration
						.first("EthosColleagueStudent", {
							filter: {
								criteria: {
									credentials: [
										{
											type: "colleaguePersonId",
											value: vm.colleagueID()
										}
									]
								}
							}
						...
						.then(function (data) {
							if (data) {
								vm.name(
									jp.query(
										data,
										"$.names[?(@.type.detail.id == '375fc097-50b3-493c-8fef-f5bb613d22c8')].fullName"
									)
								);
								vm.email(
	...
	vm.afterLoad = function afterLoad() {
		...
		if (pkg.isAtStep("WFRegistration")) {
			//Disable all inputs
			$(":input").prop("disabled", true);
			// Disable the semester input field
			$("#studentsSelectedCounselorEmail").prop("disabled", true);
			// Change cursor to 'not-allowed' or 'default' on mouse-over
			$("#studentsSelectedCounselorEmail").css("cursor", "not-allowed");
		}

		if (vm.occRole()) {
			roleRowShow(vm.occRole());
		}
	...
	function roleRowShow(roleValue) {
		switch (roleValue) {
			case "Advisor":
				$(".question1Row").show();
				$(".question2Row").hide();
