define([
    ...
    */
	...
	vm.setDefaults = function setDefaults(source, inputValues) {
		...
		if (pkg.isAtStep("EndSupervisorDecline")) {
			$(".declineHide").show();
		}
											...
											integration
												...
												.then(
													function (supervisorData) {
														vm.supervisorName(
															getPreferredName(
																supervisorData.names
															)
														);
														vm.routeSupervisorEmail(
															getPrimaryEmail(
																supervisorData.emails
															)
														);
														$(".loading").hide();
													}
	...
	vm.afterLoad = function afterLoad() {
		...
		ko.computed(function () {
			vm.vacationDatesList().forEach(function (date, index) {
				const start = date.vacatationStart();
				const end = date.vacationEnd();

				const partialDayId = `vacationDatesList__${index}__partialDay`;

				if (start && end && start === end) {
					$(`#${partialDayId}, label[for='${partialDayId}']`).show();
				} else {
					// Uncheck using the observable, if it exists
					if (ko.isObservable(date.partialDay)) {
						date.partialDay(false);
					}
					$(`#${partialDayId}, label[for='${partialDayId}']`).hide();
				}
			});
		});
	...
	vm.onSubmit = function onSubmit(form) {
		...
		vm.requestDate(Extenders.utils.formatDate(new Date()));
	};
	...
	vm.onDecline = function onDecline(form) {
		...
		if (vm.declineReason()) {
			...
			requiredIndicators.setRequired("declineReason", true);
			throw new Error(
				"Please enter a reason for declining at the bottom of the form and click Decline again to move the form to the next step."
			);
		}
	};
