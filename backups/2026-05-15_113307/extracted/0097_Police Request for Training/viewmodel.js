define([
  ...
  $(".developer").hide();
  $(".approvals").hide();
  $(".campusLieutenantSign").hide();
  $(".trainingLieutenantSign").hide();
  $(".chiefSafetySign").hide();
  $(".declineHide").hide();
  ...
  const GROUP = {
    AHLieutenant: "46e1c7ca-0fb2-4f04-81f9-9a52fa43c7b8",
    HLLieutenant: "8cb070aa-12f0-40bc-be68-170e4ead4578",
    ORLieutenant: "52a4db0e-25f4-4e9a-b858-13183ff726ea",
    ROLieutenant: "ef0b828b-80cb-407b-9efe-465354e3dc52",
    SFLieutenant: "8d5f5951-b563-45ff-8aa6-04089c8fad85",
  };
  ...
  vm.setDefaults = function setDefaults(source, inputValues) {
    ...
    //show green Approvals Banner
    if (
      pkg.isAtStep("ConditionalActorGetCampusLieutenant") ||
      pkg.isAtStep("WFPSFYTrainingLieutenant") ||
      pkg.isAtStep("WFPSFYPublicSafetyChief") ||
      pkg.isAtStep("End")
    ) {
      $(".approvals").show();
      $(".campusLieutenantSign").show();
    }
    ...
    if (
      ...
      $("#campusLieutenantSign").prop("readonly", true);
      $(".trainingLieutenantSign").show();
    }

    if (pkg.isAtStep("WFPSFYPublicSafetyChief") || pkg.isAtStep("End")) {
      $("#trainingLieutenantSign").prop("readonly", true);
      $(".chiefSafetySign").show();
    }
    if (pkg.isAtStep("EndDecline")) {
      $(".declineHide").show();
    }

    // autopopulate dates at each signature step
    vm.campusLieutenantSign.subscribe(function (newValue) {
      if (newValue) {
        vm.campusLieutenantSignDate(Extenders.utils.formatDate(new Date()));
      } else {
        vm.campusLieutenantSignDate("");
      }
    });

    vm.trainingLieutenantSign.subscribe(function (newValue) {
      if (newValue) {
        vm.trainingLieutenantSignDate(Extenders.utils.formatDate(new Date()));
      } else {
        vm.trainingLieutenantSignDate("");
      }
    });

    vm.chiefSafetySign.subscribe(function (newValue) {
      if (newValue) {
        vm.chiefSafetySignDate(Extenders.utils.formatDate(new Date()));
      } else {
        vm.chiefSafetySignDate("");
      }
    });

    vm.campus.subscribe(function (newValue) {
      switch (newValue) {
        case "Auburn Hills":
          vm.groupPrincipalID(GROUP.AHLieutenant);
          break;
        case "Highland Lakes":
          vm.groupPrincipalID(GROUP.HLLieutenant);
          break;
        case "Orchard Ridge":
          vm.groupPrincipalID(GROUP.ORLieutenant);
          break;
        case "Royal Oak":
          vm.groupPrincipalID(GROUP.ROLieutenant);
          break;
        case "Southfield":
          vm.groupPrincipalID(GROUP.SFLieutenant);
          break;
        default:
          vm.groupPrincipalID("");
          break;
    ...
    if (user.isInLibrary) {
      ...
      // vm.originatorEmail(user.originatorEmail);
    } else {
  ...
  vm.onDecline = function onDecline(form) {
    // don't require signatures if declining
    requiredIndicators.setRequired("campusLieutenantSign", false);
    requiredIndicators.setRequired("trainingLieutenantSign", false);
    requiredIndicators.setRequired("chiefSafetySign", false);
  ...
  vm.afterRequired = function afterRequired(form) {
    ...
    if (form.attachmentCount < 1) {
      notify("error", "Please attach documentation for this training.");
      return false;
    } else {
      return true;
    }
