...
define([
    ...
    var tableShow = ['XHR to Review', 'XPayroll Hold', 'XPayroll Processing', 'XHR Hold'];     //list to expand, remove to collapse
    var displayTable = [];
    ...
    $('#refreshBtn').click(function () {
        ...
        integration.all('Dashboard_HR_Payroll').then(function(contentData) {
        ...
        documents.forEach(function (doc, index) {
            ...
            documentConfig.forEach(function(configObj) {
              ...
              //  if(configObj.docTypeArray.includes(doc.DocumentType) === true){
		        ...
		        if (configObj.title == 'Payroll Hold' && doc.HRReviewStatus == 'Payroll Hold') {
                     pushDocument(configObj, doc);
                            }
                else if (configObj.title == 'HR Hold' && doc.HRReviewStatus == 'HR Hold') {
                     pushDocument(configObj, doc);
                            }
                else if (configObj.title == 'HR to Review' && doc.HRReviewStatus == 'HR to Review') {
                     pushDocument(configObj, doc);
                            }
                else if (configObj.title == 'Payroll Processing' && doc.HRReviewStatus == 'Payroll Processing') {
                     pushDocument(configObj, doc);
                            }
    ...
    function drawTable(config, index) {
       ...
       var $theader = $('<thead>');
        ...
        stats.employeeCount = new Set(config.documents.map( doc => doc.EmployeeLookup)).size;
        ...
        $statRow.append(
            $('<div class="col-xs-5 stats">')
                .html(
                    `${stats.employeeCountLbl} <span id="${stats.tableName}employeeCount"><strong>${stats.employeeCount}</strong></span> | 
        ...
        $('#dataTable[data-note="' + noteID + '"]').on('filterEnd sortEnd', function () {
            ...
            // strip spaces to use as an ID prefix for Employee and document counts
            //note = note.replace(/ /g, '');

            let visibleRows = $(this).find('tr').not('.filtered, .tablesorter-headerRow, .tablesorter-ignoreRow');

            let employeeNumbers = visibleRows.toArray().map(function (row) {
                let td = $(row).find('td').get(5) // UPDATE this to the column number for Employee ID or Employee Number. First column is 0
                return $(td).html()
            });

            // a Set cannot have multiple entries - this serves as an easy filter to count Employees.
            let EmployeeCount = (new Set(employeeNumbers)).size;

            $(`#${note}EmployeeCount strong`).html(EmployeeCount);
    ...
    function buildFullStats(data) {
        ...
        stats.employeeCount = new Set(data.map( doc => doc.EmployeeLookup)).size;

        $('#fullStats')
            .css('font-weight', 'bold')
            .text(
                'Total: ' +
                stats.employeeCountLbl + ' ' +
                stats.employeeCount + ' | ' +
