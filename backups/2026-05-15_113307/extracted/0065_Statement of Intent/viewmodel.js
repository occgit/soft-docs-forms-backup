    ...
    vm.term.subscribe(function (newValue) {
      ...
      if (newValue) {
        ...
        if (matchingCourses.length === 0) {
          notify("error", "No registered course found for the selected term.");
        ...
        matchingCourses.forEach(function (regByTermRow, i) {
          if (regByTermRow.registrationTerm == newValue) {
            ...
            vm.courseTable()[i].courseCode(regByTermRow.registrationCourseCode);
            vm.courseTable()[i].courseName(regByTermRow.registrationCourseName);
            vm.courseTable()[i].creditHours(regByTermRow.registrationCredits);
          }
        });
      }
    });
            ...
            .then(function (registrationResults) {
              ...
              registrationResults.forEach(
                function (registrationSection, index) {
                  if (
                    registrationSection.status.registrationStatus ==
                      "registered" &&
              ...
              Promise.all(p).then(function (sectionResults) {
                ...
                for (const regResult of registrationResults) {
                  for (const secResult of sectionResults) {
                    if (regResult.section.id === secResult.id) {
                      ...
                      const sectionNumber = secResult.number;
                      const courseName = secResult.titles[0].value;
                      const credits = secResult.credits[0].minimum;
                ...
                SectionRegistration.forEach(function (Results) {
                  vm.studentCourseReg.push({
                    ...
                    registrationSectionNumber: Results.sectionNumber,
                    registrationCourseName: Results.courseName,
  ...
  vm.afterRequired = function afterRequired(form) {
    ...
    if (pkg.isAtStep("Start")) {
      if (isThereCourseSelectedTerm === false) {
        throw new Error("No registered course found for the selected term.");
      }
    }
  };
