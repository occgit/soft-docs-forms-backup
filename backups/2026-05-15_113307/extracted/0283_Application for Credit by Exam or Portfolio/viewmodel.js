define([
  ...
  vm.addObservableArray("testingCenterOptions");
  vm.orchardTestingCenter = ko.observable();
  vm.auburnTestingCenter = ko.observable();
  ...
  function readOnlyFields() {
    $("#additionalEmail").prop("disabled", true);
    $("#additionalPhone").prop("disabled", true);

    $("#otherCourse").prop("disabled", true);
    $("#studentSignature").prop("disabled", true);
    autoCompleteUtils.makeReadOnly("course");
  }

  function adminReadOnlyFields() {
    autoCompleteUtils.makeReadOnly("semester");
    $("#section").prop("disabled", true);
    $("#testingCenter").prop("disabled", true);
    $("#adminNotes").prop("disabled", true);
    $("#academicYear").prop("disabled", true);
    autoCompleteUtils.makeReadOnly("facultyPointPerson");
    autoCompleteUtils.makeReadOnly("courseCode");
  ...
  vm.setDefaults = function setDefaults(source, inputValues) {
    ...
    integration
      ...
      .catch(function (error) {
        ...
        notify("error", "There was an issue loading academic periods.");
      });
    ...
    vm.employeeSignature.subscribe(function (sig) {
      vm.employeeSignDate(getTodayDate());
    });

    vm.section.subscribe(function (section) {
      var course = vm.courseCode();
      vm.courseSection(course + " " + section);
    });

    vm.testingCenter.subscribe(function (center) {
      vm.selectedTestingCenter(center);
    });

    vm.course.subscribe(function (selectedCourse) {
      if (selectedCourse == "CNL | Course Not Listed") {
        $(".cnl").show();
      } else {
        $(".cnl").hide();
      }
    ...
    if (pkg.isAtStep("ChairCoordinator") || pkg.isAtStep("Dean")) {
      //can use internal notes and history as signature
      $(".admin").hide();
      $(".faculty").hide();
      $(".registrar").hide();
    ...
    if (pkg.isAtStep("AdminSpecialistCourseCreate")) {
      ...
      populateTestingCenterDropDown();
    }

    if (pkg.isAtStep("EnrollmentServices")) {
      //only needs to register student and approve
      $(".admin").show();
      $(".faculty").hide();
      $(".registrar").hide();
    ...
    if (pkg.isAtStep("AdminSpecialistFeePay")) {
      //the admin will monitor the fee payment for the students and approve once
    ...
    if (pkg.isAtStep("FacultyPointPerson")) {
      $(".admin").show();
      $(".faculty").show();
      $(".registrar").hide();
    ...
    if (pkg.isAtStep("Registrar")) {
      ...
      $("#courseSection").prop("disabled", true);
      $("#letterGrade").prop("disabled", true);
    }
  };
  ...
  vm.onSubmit = function onSubmit(form) {
    ...
    if (pkg.isAtStep("Start")) {
      if (vm.course() == "CNL | Course Not Listed") {
        requiredIndicators.setRequired("otherCourse", true);
      } else {
        requiredIndicators.setRequired("otherCourse", false);
      }
    }
  };
  ...
  vm.onApprove = function onApprove(form) {
    ...
    if (pkg.isAtStep("AdminSpecialistCourseCreate")) {
      requiredIndicators.setRequired("semester", true);
      requiredIndicators.setRequired("section", true);
      requiredIndicators.setRequired("testingCenter", true);
      requiredIndicators.setRequired("facultyPointPerson", true);
      requiredIndicators.setRequired("academicYear", true);
    }

    if (pkg.isAtStep("FacultyPointPerson")) {
      requiredIndicators.setRequired("letterGrade", true);
      $("#courseSection").prop("disabled", true);
    }
  };
  ...
  function populateTestingCenterDropDown() {
    var auburn = vm.auburnTestingCenter();
    var orchard = vm.orchardTestingCenter();

    vm.testingCenterOptions.removeAll();

    if (orchard == "Y" && auburn == "Y") {
      vm.testingCenterOptions.push(
        { testCenter: "Auburn Hills" },
        { testCenter: "Orchard Ridge" },
      );
    } else if (orchard == "N" && auburn == "Y") {
      vm.testingCenterOptions.push({ testCenter: "Auburn Hills" });
    } else if (orchard == "Y" && auburn == "N") {
      vm.testingCenterOptions.push({ testCenter: "Orchard Ridge" });
    }
  }
