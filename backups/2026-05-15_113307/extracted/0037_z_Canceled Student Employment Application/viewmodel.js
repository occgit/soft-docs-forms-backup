define(['jquery',
    ...
    */
    ...
    vm.setDefaults = function setDefaults(source, inputValues) {
        ...
        vm.applicantSign.subscribe(function(newValue){
            if (newValue) {
                vm.applicantSignDate(Extenders.utils.formatDate(new Date()));
            } else {
                vm.applicantSignDate('');
            }
        });
        ...
        integration.all('EthosColleagueStudentByErpId', {
        }).then((studentInfo) => {
            console.log('Student Info: ',studentInfo); 
            const preferredName = studentInfo.names.find(name => name.preference === "preferred");
            vm.firstName(preferredName.firstName);
            vm.lastName(preferredName.lastName);
            vm.middleName(preferredName.middleName);
            if (studentInfo.child.addressPrimary.addressLines.length > 1) {
                vm.studentAddress(studentInfo.child.addressPrimary.addressLines[0] + ' ' + studentInfo.child.addressPrimary.addressLines[1]);
            } else {
               vm.studentAddress(studentInfo.child.addressPrimary.addressLines[0]); 
            }
           vm.studentCity(studentInfo.child.addressPrimary.place.country.locality);
           vm.studentState(studentInfo.child.addressPrimary.place.country.region.title);
           vm.studentZipCode(studentInfo.child.addressPrimary.place.country.postalCode);
           const studentGUID = studentInfo.id;
        //   console.log('Student GUID: ',studentGUID );
        //     integration.first('Web_Ethos_Get_Emergency_Contact_Information_by_ErpId', {
