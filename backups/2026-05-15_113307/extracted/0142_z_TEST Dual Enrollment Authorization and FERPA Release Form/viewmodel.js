...
define(['jquery',
    ...
    integration.all('Etrieve_Security_Get_Active_Employees', {
    ...
    */
    ...
    vm.setDefaults = function setDefaults(source, inputValues) {
        ...
        //  afterLoad is called.
            ...
            vm.highSchoolName.subscribe(function (hsVal) {
                hsTypeRowShow(hsVal)
            });
            ...
            //     $('.schoolCourseRow').hide();
                ...
                for (let i = 1; i <= 4; i++) {
                  ...
                  if (row) {
                    row.style.display = (value && value.toLowerCase() === 'n/a') ? 'none' : '';
                  }
                }
              }
            } else {
              $('.schoolRow').hide();
                ...
                if (vm.highSchoolName()){
                    ...
                    vm.highSchoolName.subscribe(function(NewValue) {
                        if (NewValue) { // If there's a new value
            ...
            integration.all('Web_Ethos_Get_Academic_Periods_by_startOn_date',{
                    logic: '$gte',
                ...
                if (newVal) {
                    $('.loading').show();
                        getAllDataFromPagingSource('Web_Sources_Get_Courses_by_SubjectGUID',newVal,100).then(listOfCoursesBySubjectData => {
                          console.log('listOfCoursesBySubjectData',listOfCoursesBySubjectData);
                          ...
                          listOfCoursesBySubjectData.forEach(function (listOfCoursesBySubjectDataRow) {
                              if (listOfCoursesBySubjectDataRow.status.id === '73c76722-884e-43e9-b093-8ebe092c045f' && listOfCoursesBySubjectDataRow.credits[0].creditCategory.creditType === 'institution') {
                                      ...
                                      vm.CourseSubjectAutocomplete.push({
                                          ...
                                          courseSubjectAndNumber: vm.subjectSearch() + ' ' + listOfCoursesBySubjectDataRow.number
                
                                      });
                              }

                          })
                          console.log('CourseSubjectAutocomplete',vm.CourseSubjectAutocomplete())
                          $('.loading').hide();
                        }).catch(error => {
                          console.error(error);
                        });
            ...
            ko.computed(function () {
                ...
                for (i = 0; i < 4; i++) {
                    ...
                    if (row && row.studentSelectedCourse()) {
                        ...
                        vm['schoolCourseAuthorizationCourseTitle' + (i + 1)](title);
                        vm['schoolCourseAuthorizationPayment' + (i + 1)](undefined);
                    } else {
                        vm['schoolCourseAuthorizationCourse' + (i + 1)](undefined);
                        vm['schoolCourseAuthorizationCourseTitle' + (i + 1)](undefined);
                    ...
                    ko.computed(function () {
                        ...
                        for (i = 0; i < 4; i++) {
                            ...
                            if (row && row.studentSelectedCourse()) {
                                ...
                                vm['schoolCourseAuthorizationCourseTitle' + (i + 1)]('n/a');
                            }
                        }
                    });
                }
    ...
    vm.afterLoad = function afterLoad() {
        ...
        if (pkg.isAtStep('HighSchoolESignStep') || pkg.isAtStep('AdmissionsESignStep') || pkg.isAtStep('WFFinancialServices')) {
            ...
            if (pkg.stepCode === 'HighSchoolESignStep' || pkg.stepCode === 'DistrictLevelReview') {
                    //ko.computed(function () {
                        var row, i;
                        for (i = 0; i < 4; i++) {
                            //row = (vm.studentSelectedCoursesRow()[i] || null);
    ...
    vm.onESignSubmit = function onESignSubmit(form) {
        ...
        if (pkg.isAtStep('ParentESignStep')) {
            if ((vm.enrollmentType() == 'Self-Pay' && !vm.parentAgreement1()) || (vm.enrollmentType == 'Sponsored by High School/District' && !vm.parentAgreement2())) {
                throw new Error('You must check the box below Select Appropriate Payment Option to proceed.');           
            } else {
                return true;
            }
        }
...
//==================== HELPER FUNCTIONS =======================================
            ...
            function validateCourseAuthorizations() {
              ...
              for (let i = 1; i <= 4; i++) {
                const courseField = document.getElementById(`schoolCourseAuthorizationCourse${i}`);
                const paymentField = document.getElementById(`schoolCourseAuthorizationPayment${i}`);
                ...
                if (courseField && paymentField) {
                  ...
                  if (courseValue && !paymentValue) {
                    throw new Error(`Since ${labelCourse} ${i} is completed, ${labelPayment} ${i} must be completed.`);
                  }
                }
              }
            }
