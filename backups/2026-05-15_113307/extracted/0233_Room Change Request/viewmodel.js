define([
  ...
  function makeAllFieldsReadOnly() {
    ...
    $("#sectionType").prop("disabled", true);
    ...
    autoCompleteUtils.makeReadOnly("instructor");
    autoCompleteUtils.makeReadOnly("#course");
    autoCompleteUtils.makeReadOnly("#section");
    autoCompleteUtils.makeReadOnly("#registrationCode");

    autoCompleteUtils.makeReadOnly("startTime");
    autoCompleteUtils.makeReadOnly("endTime");
    autoCompleteUtils.makeReadOnly("#numberOfWeeks");
    autoCompleteUtils.makeReadOnly("#currentRoom");
    autoCompleteUtils.makeReadOnly("#requestedRoom");
    autoCompleteUtils.makeReadOnly("#effectiveDate");
    autoCompleteUtils.makeReadOnly("#changeReason");
  }
  ...
  vm.setDefaults = function setDefaults(source, inputValues) {
    ...
    if (user.isInLibrary) {
      ...
      vm.instructor($("#originatorDisplayName").val());
      vm.instructorEmail($("#originatorEmail").val());
    } else {
    ...
    // vm.admin.subscribe(function (adminEntry) {
    //   vm.administrativeSpecialistEmail(adminEntry);
    // });
    ...
    if (
      pkg.isAtStep("FacilitiesAH") ||
      pkg.isAtStep("FacilitiesHL") ||
      pkg.isAtStep("FacilitiesOR") ||
      pkg.isAtStep("FacilitiesROSF")
    ) {
      $(".admin").hide();
      $(".facilities").show();
    ...
    if (pkg.isAtStep("CondAdmin")) {
      $(".admin").show();
      $(".facilities").show();

      makeAllFieldsReadOnly();
      //Facilities Use
      $("#actualRoomAssignment").prop("disabled", true);
      autoCompleteUtils.makeReadOnly("admin");
  ...
  vm.onSubmit = function onSubmit(form) {
    ...
    const originatorEmail = $("#originatorEmail").val();
    const instructorEmail = $("#instructorEmail").val();
    if (instructorEmail == originatorEmail) {
      vm.isOriginatorInstructor("Yes");
    } else {
      vm.isOriginatorInstructor("No");
    }
    ...
    if (
      ...
      notify("error", "Days of the week is required");
      return false;
    }
  };
  ...
  vm.onApprove = function onApprove(form) {
    ...
    if (
      ...
      $("#actualRoomAssignment").prop("disabled", false);
    ...
    if (pkg.isAtStep("CondAdmin")) {
      //Admin Use
      $("#facultySecretary").prop("disabled", false);
      $("#dean").prop("disabled", false);
      $("#deptChair").prop("disabled", false);
      $("#cardDone").prop("disabled", false);
    }
  };
