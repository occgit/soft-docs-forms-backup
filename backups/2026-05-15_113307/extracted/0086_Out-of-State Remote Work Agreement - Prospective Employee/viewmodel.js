define([
  ...
  vm.setDefaults = function setDefaults(source, inputValues) {
    ...
    if (
      pkg.isAtStep("SupervisorESign") ||
      pkg.isAtStep("WFOSWHR") ||
      pkg.isAtStep("WFOSWFinancialServices") ||
      pkg.isAtStep("WFOSWIT") ||
      pkg.isAtStep("EmployeeESign") ||
      pkg.isAtStep("OSWHRLookupCheck") ||
      pkg.isAtStep("End") ||
      pkg.isAtStep("EndNoID") ||
      pkg.isAtStep("ESignLookupCheck")
    ) {
    ...
    if (pkg.isAtStep("ESignLookupCheck")) {
      $(".lookupCheck").show();
    }
    ...
    if (user.isInLibrary) {
      ...
      vm.hrDocType("Out-of-State Remote Work Agreement - Prospective Employee");
  ...
  vm.onESignSubmit = function onESignSubmit(form) {
    ...
    if (pkg.isAtStep("ESignLookupCheck")) {
      if (lookupCheckTracker == "No") {
        throw new Error(
          "Form cannot be submitted until a Lookup has been created for this Employee. Please try again later.",
        );
  ...
  function lookupCheck() {
    integration
      .first("Etrieve_Content_Connection_Employee_Lookup_Check", {
      ...
      .then(function (vals) {
        if (!vals) {
          ...
          notify(
            "error",
            "No Lookup found for this employee. There will need to be an Employee Lookup before you can approve this Remote Work Agreement.",
          );
        } else {
          ...
          notify(
            "success",
            "Emploee Lookup found! You may now approve this Remote Work Agreement.",
          );
        }
      });
  }
