define([
  ...
  integration
    ...
    .then(function (data) {
      console.log("EMPLOYEES FROM SECURITY: ");
    });
  /*
  ...
  vm.setDefaults = function setDefaults(source, inputValues) {
    ...
    vm.campus.subscribe(function (campus) {
      integration.all("HR_Routing_Data", {}).then(function (data) {
        const facilitiesEmail = data
          .filter((row) => row.Campus === campus && row.Role === "Facilities")
          .map((row) => row.Email);

        vm.facilitiesEmail(facilitiesEmail);

        const campusITEmail = data
          .filter((row) => row.Campus === campus && row.Role === "Campus IT")
          .map((row) => row.Email);
    ...
    // Only set default supervisor values when the form starts
    if (pkg.isAtStep("Start")) {
      if (!vm.supervisorName()) {
        vm.supervisorName(user.DisplayName);
      }

      $(".hrView").hide();
    } else {
      // Lock the fields after Start so later workflow users do not overwrite them
      $("#supervisorName").prop("readonly", true);
      $("#supervisorPhone").prop("readonly", true);

      // optional visual cue
      $("#supervisorName, #supervisorPhone").css({
    ...
    integration
      ...
      .then(function (personData) {
        if (!personData) {
          ...
          vm.employeeName(
            personData.names[0].firstName + " " + personData.names[0].lastName,
          );
