define([
  ...
  vm.setDefaults = function setDefaults(source, inputValues) {
    ...
    //Collecting advisor name from Autocomplete (pass to integration)
    vm.advisor.subscribe(function (advName) {
      vm.advisorNameHidden(advName);
    });

    // ============ CLEAR OUT DATA ASSOCIATED WITH AFFILIATION IF AFFILIATION INPUT CHANGES =================
    vm.affiliation.subscribe(function () {
      vm.orgName("");
      vm.orgWebsite("");
      vm.taxExempt("");
      vm.taxExemptID("");
    });
    // ============ END CLEAR OUT DATA ASSOCIATED WITH AFFILIATION IF AFFILIATION INPUT CHANGES =============

    // ========================= SHOW CURRENT DATE WHEN NAME IS ENTERED ======================================
    ...
    // ========================= END SHOW CURRENT DATE WHEN NAME IS ENTERED ==================================

    // ============================= SHOW AND HIDE SIGNATURE SECTIONS BASED ON WORKFLOW STEP ======================================
    if (
    ...
    // ============================= END SHOW AND HIDE SIGNATURE SECTIONS BASED ON WORKFLOW STEP ==================================
    ...
    if (user.isInLibrary) {
      ...
      vm.secDocType("Application for New RSO");
    ...
    if (!user.isInLibrary && !user.isInDrafts) {
      $(document).ready(function () {
        setTimeout(function () {
          ...
          $("#advisor").prop("disabled", true);

          // Change cursor to 'not-allowed' or 'default' on mouse-over
          $("#advisor").css("cursor", "not-allowed");
  ...
  //     let promiseArray = [];
  //     if (pkg.isAtStep('WFRSOStudentEngagementCoordinators')) {
  //         promiseArray.push (
  //             integration.add('Add_RSO_Info', {
  //                 proposedRSOName: vm.proposedRSOName(),
  //                 advisor:         vm.advisor(),
  //                 advisorEmail:    vm.advisorEmail(),
  //                 advisorPhone:    vm.advisorPhone()
  //             })
  //         )
  //         vm.advisorRow.forEach(function(advisorVals) {
  //             promiseArray.push (
  //                 integration.add('Add_RSO_Info', {
  //                     proposedRSOName: vm.proposedRSOName(),
  //                     advisor:         advisorVals.additionalAdvisor(),
  //                     advisorPhone:    advisorVals.additionalAdvisorPhone(),
  //                     advisorEmail:    advisorVals.additionalAdvisorEmail()
  //                 });
  //             )
  //         });
  ...
  vm.afterRequired = function afterRequired(form) {
    ...
    if (pkg.isAtStep("WFRSOStudentEngagementCoordinators")) {
      promiseArray.push(
        integration.add("Add_RSO_Info", {
          proposedRSOName: vm.proposedRSOName(),
          advisor: vm.advisorNameHidden(),
          advisorEmail: vm.advisorEmail(),
          advisorPhone: vm.advisorPhone() || "",
        }),
      );

      let isThereAdditionalAdvisor = vm.additionalAdvisors();

      console.info("Is there additional advisors?", isThereAdditionalAdvisor);

      if (isThereAdditionalAdvisor === "Yes") {
        vm.advisorRow.forEach(function (advisorVals) {
          advisorVals.additionalAdvisorNameHidden(
            advisorVals.additionalAdvisor(),
          );
          promiseArray.push(
            integration
              .add("Add_RSO_Info", {
                proposedRSOName: vm.proposedRSOName(),
                advisor: advisorVals.additionalAdvisorNameHidden(),
                advisorPhone: advisorVals.additionalAdvisorPhone() || "",
                advisorEmail: advisorVals.additionalAdvisorEmail(),
              })
              .catch(function (error) {
                console.error("Error adding additional Advisor: ", error);
              }),
          );
        });
      }
