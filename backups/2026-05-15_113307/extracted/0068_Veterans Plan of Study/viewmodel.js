define([
  ...
  vm.setDefaults = function setDefaults(source, inputValues) {
    ...
    if (
      pkg.isAtStep("ConditionalCounselor") ||
      pkg.isAtStep("VAStaff") ||
      pkg.isAtStep("PietraOliveira") ||
      pkg.isAtStep("End") ||
      pkg.isAtStep("ConditionalCounselor2")
    ) {
      $(".counselor").show();
    }
    ...
    if (!pkg.isAtStep("ConditionalCounselor")) {
      $(".counselor, #counselorSignature, #extension").prop("readonly", true);
      $("#degreeProgram")
        .prop("readonly", true)
        .attr({ tabindex: "-1", "aria-disabled": "true" })
        .css({
      ...
      $("#catalogYear, #campus").prop("disabled", true);
    }
    if (user.isInLibrary) {
      ...
      vm.vaDocType("Veterans Plan of Study");
  ...
  vm.onApprove = function onApprove(form) {
    ...
    if (pkg.isAtStep("ConditionalCounselor")) {
      if (form.attachmentCount < 1) {
        notify("error", "You must attach a document before proceeding.");
        return false;
      } else {
        return true;
      }
