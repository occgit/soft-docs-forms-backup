...
define([
    ...
    vm.onLoad = function onLoad(source, inputValues){
        ...
        $('#originatorErpId').attr('readonly',true);
        $('#originatorEmail').attr('readonly',true);
        
               // document.getElementById('AcademicYear').style.display = 'none';
         document.getElementById('AcademicYear').hidden = 'true';  
        // document.getElementById('InputHint').hidden = 'true';   
        
        // Get the element with the ID 'AcademicYear'
        let originatorElement = document.getElementById('AcademicYear');
        // Check if the element exists
        if (originatorElement) {    
        // Traverse to the previous sibling element
        let previousElement = originatorElement.previousElementSibling;
        // Loop to find the previous <div class="inputHint">
        while (previousElement) {        if (previousElement.classList.contains('inputHint')) {
        // Found the previous <div class="inputHint">
        previousElement.hidden = true; // Hide the element
        break;        }        previousElement = previousElement.previousElementSibling;  
        } 
    ...
    vm.setDefaults = function setDefaults(source, inputValues){
        ...
        $('#studentSport, #todayDate, #studentName, #dateOfBirth, #studentCollegeAddress, #studentPhoneNumberAtCollege, #parentHomeAddress, #parentPhoneNumber, #parentName, #highSchoolName, #highSchoolCityStateCountry, #highSchoolGradDate, #dateGEDEarned, #studentAthleteSignature, #studentAthleteSignatureDate').prop('required', 'true');
        ...
        if (user.isInLibrary){
            ...
            $('#todayDate').mask("99/99/9999");
            $('#dateOfBirth').mask("99/99/9999");
            $('#highSchoolGradDate').mask("99/99/9999");
            $('#collegeDateQuestion2').mask("99/99/9999");
            $('#dateGEDEarned').mask("99/99/9999");
            $('#collegeOneDate').mask("99/99/9999");
            $('#collegeTwoDate').mask("99/99/9999");
            $('#collegeThreeDate').mask("99/99/9999");
            $('#collegeFourDate').mask("99/99/9999");
            $('#studentAthleteSignatureDate').mask("99/99/9999");
            $('#studentPhoneNumberAtCollege').mask("(999) 999-9999");
            $('#parentPhoneNumber').mask("(999) 999-9999");
