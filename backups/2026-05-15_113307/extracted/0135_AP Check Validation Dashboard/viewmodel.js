...
define([
    ...
    var tableShow = ['Check Validation'];   // default table to show on initial load. must match the config title.
    var displayTable = [];
    ...
    $('#refreshBtn').click(function () {
        ...
        integration.all(integrationName)
    ...
    function splitAndDraw(documents) {
        ...
        // GROUP BY DocumentID + MERGE Invoice Numbers
        let mappedDocuments = documents.reduce(function (mappedDocs, doc) {
            ...
            if (found) {
                let invoice = String(doc.InvoiceNumber || '').trim();

                if (invoice && !found.InvoiceNumbers.includes(invoice)) {
                    found.InvoiceNumbers.push(invoice);
                }
            } else {
                let newDoc = Object.assign({}, doc);

                let invoice = String(doc.InvoiceNumber || '').trim();
                newDoc.InvoiceNumbers = invoice ? [invoice] : [];
        ...
        // flatten invoices into comma-separated string
    ...
    function ensureScrollTopButton() {
        if ($('#stickyScrollTopBtn').length) return;

        const $btn = $('<button>', {
            id: 'stickyScrollTopBtn',
            type: 'button',
            html: '↑ Top'
        })
