define([
  ...
  vm.setDefaults = function setDefaults(source, inputValues) {
    ...
    if (!pkg.isAtStep("Start")) {
      autoCompleteUtils.makeReadOnly("firstApproverSearch");
      requiredIndicators.setRequired("poBPO", true);
    }

    if (pkg.isAtStep("ConditionalActorSecondApprover")) {
      autoCompleteUtils.makeReadOnly("secondApproverSearch");
      $("#secondApproverQuestion").prop("disabled", true);
    }
    ...
    vm.poBPO.subscribe(function (newValue) {
      if (newValue == null || newValue === "") return;

      const validated = validatePoBpo(newValue);

      if (validated === null) {
        notify("error", "PO/BPO must be 'P' or 'B' followed by 5 numbers.");
        vm.poBPO("");
      } else {
        vm.poBPO(validated);
      }
        ...
        Promise.all(promiseArrays).then(function (allMyPromisesReturned) {
          if (allMyPromisesReturned[0]) {
            ...
            const targetAddress = addresses
              .filter((addr) => addr.type.addressType === "billing")
            ...
            if (targetAddress)
              vm.organizationAddressId(targetAddress.address.id);
  ...
  vm.beforeRequired = function beforeRequired(form) {
    ...
    if (!pkg.isAtStep("Start")) {
      const poValue = validatePoBpo(vm.poBPO());
      if (vm.poBPO() && poValue === null) {
        throw new Error("PO/BPO must begin with P or B followed by 5 numbers.");
      }
    }
  };
  ...
  function parseAmount(value) {
    ...
    function validatePoBpo(value) {
      ...
      // Match P or B followed by 1-7 digits
      const match = trimmed.match(/^([PB])(\d{1,7})$/i);
      if (match) {
        const letter = match[1].toUpperCase();
        const digits = match[2].padStart(7, '0');
...
//     // Must be P or B followed by exactly 5 digits
//     if (/^[PB]\d{5}$/i.test(trimmed)) {
//       return trimmed.toUpperCase();
//     }
