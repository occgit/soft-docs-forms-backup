...
define([
    ...
    var tableShow = ['Ready to Voucher'];   // default table to show on initial load. must match the config title.
    var displayTable = [];
       ...
       /* let mappedDocuments = documents.reduce((mappedDocs, doc, index, documents) => {
        ...
        documents.forEach(function (doc, index) {
            ...
            documentConfig.forEach(function(configObj) {
				...
				if (configObj.title == 'Ready to Voucher' && doc.InvoiceStatus == 'Ready to Voucher') {
                     pushDocument(configObj, doc);
                            }
                else if (configObj.title == 'Invoice Hold' && doc.InvoiceStatus == 'Hold') {
                     pushDocument(configObj, doc);
                            }
                else if (configObj.title == 'Invoice Received' && doc.InvoiceStatus == 'Received') {
                     pushDocument(configObj, doc);
                            }
