...
define([
...
], function viewmodel($, ko, vm, user, integration, toastr) {
    ...
    var tableShow = ['FA Advisors A-G', 'FA Advisors H-N', 'FA Advisors O-Z'];
    // var tableShow = [];
    var displayTable = [];
    ...
    $('#refreshBtn').click(function () {
        ...
        integration.all('Dashboard_FA_Advisors', {}).then(function(contentData) {
        ...
        documents.forEach(function (doc, index) {
            ...
            documentConfig.forEach(function(configObj) {
                ...
                if(configObj.docTypeArray.includes(doc.DocumentType) === true){
                     ...
                     if(doc.StudentLName.match(/^[A-G]{1}/)){
                             if(configObj.title ===  'FA Advisors A-G'){
                                 configObj.documents.push(doc);             
                             }
                      }
                     else if(doc.StudentLName.match(/^[H-N]{1}/)){
                             if(configObj.title ===  'FA Advisors H-N'){
                                 configObj.documents.push(doc);             
                             }
                      }   
                     else if(doc.StudentLName.match(/^[O-Z]{1}/)){
                             if(configObj.title ===  'FA Advisors O-Z'){
                                 configObj.documents.push(doc);             
                             }
                      }      
                    else {
    ...
    function drawTable(config, index) {
       ...
       var $theader = $('<thead>');
        ...
        // count documents
        console.log("Student ID Sample", config.documents[0]?.StudentNumber)
