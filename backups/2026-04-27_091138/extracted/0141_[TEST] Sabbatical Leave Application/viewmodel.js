...
define(['jquery',
    ...
    $('#sendToCommittee').click(function () {
        vm.sentToCommittee('Yes');
...
//============================= DO STUFF BASED ON WORKFLOW STEP ===================================
        ...
        if (pkg.isAtStep('ConditionalExecutiveCouncilMember')) {
            ...
            console.log('Exec Council Display Name: ', vm.execCouncilDisplayName());
        }
        ...
        if (pkg.isAtStep('WFHRChancellor') || pkg.isAtStep('EndApproved') || pkg.isAtStep('EndDeclined')) {
            $('.signatures').show();
            $('.supervisor').show();
            $('.departmentChair').show(); 
            $('.executiveCouncilMember').show();
...
//=========================== END DO STUFF BASED ON WORKFLOW STEP =================================
        ...
        vm.departmentGUID.subscribe(function(newValue) {
            if (newValue) { 
                integration.all('Web_Ethos_Get_Employment_Department_by_GUID', {
...
//=========================== SUBSCRIBES FOR SIGNATURE FIELDS ===================================
        ...
        vm.execCabMemberSign.subscribe(function(newValue) {
            if (newValue) {
                vm.execCabMemberSignDate(Extenders.utils.formatDate(new Date()));                
            } else {
                vm.execCabMemberSignDate('');
            }
        });
...
//========================= END SUBSCRIBES FOR SIGNATURE FIELDS ===================================
        ...
        if (user.isInLibrary || user.isInDrafts) {
            ...
            integration.all('Etrieve_Security_Get_Chancellor_Admins', {
                ...
                //console.log('Admin Emails: ',vm.chancellorAdminEmails());
    ...
    vm.afterLoad = function afterLoad() {
        ...
        ko.computed(function() {
            ...
            vm.committeeMembersList.forEach(function(member) {
                if (member.committeeMemberEmail()) {
                    emails.push(member.committeeMemberEmail());
                }
            });
        
            let memberEmails = emails.join('; ');
            // Do something with memberEmails, like assign to an observable or return it
            console.log(memberEmails);
            vm.committeeEmails(memberEmails);
        });

        if(vm.committeeMembersList().length < 1) {
           vm.committeeMembersListAdd();
        }
    ...
    vm.afterRequired = function afterRequired(form) {
        ...
        if(form.attachmentCount < 1){ 
           notify('error','You must attach a Sabbatical Leave Proposal before submitting this form.'); 
           return false; 
        }else{ 
           return true; 
        } 
        ...
        if (pkg.isAtStep('ConditionalExecutiveCouncilMember')) {
            if (vm.sentToCommittee() == 'No') {
               notify('error','You must click the button to notify the committee before moving this form alonog in the workflow.'); 
               return false;                 
            }
        }
       ...
       //function to sort through all names and return the preferred name
        ...
        function submitFormsAPI() {
            ...
            apiCalls.forEach(call => {
              ...
              p_FormDraftCreation.push(
                  apiRequest('POST', url, payloadCall)
              );
            });
            ...
            Promise.all(p_FormDraftCreation)
              .then(draftResults => {
                draftResults.forEach((draftResponse, index) => {
                  ...
                  console.log(`Create Form Draft API Response ${index + 1}:`, draftResponse);
                    ...
                    if (apiCalls[index].formData.length > 0) {
                      ...
                      p_FormDataUpdate.push(
                          apiRequest('PUT', urlPUT, payload)
                      );
                    }
                });
                ...
                Promise.all(p_FormDataUpdate)
                    .then(dataResults => {
                        dataResults.forEach((dataResponse, dataIndex) => {
                            // Handle successful API response
                            console.log(`Create Form Data API Response ${dataIndex + 1}:`, dataResponse);
                        });
                        ...
                        Promise.all(p_FormSubmit)
                            .then(submitResults => {
                                submitResults.forEach((submitResponse, submitIndex) => {
                                    // Handle successful API response
                                    console.log(`Submit Form API Response ${submitIndex + 1}:`, submitResponse);
                                });
