	...
	vm.setDefaults = function setDefaults(source, inputValues) {
		...
		vm.highSchoolName.subscribe(function (option) {
			if (option == "OEC - Oakland Early College") {
				...
				console.log(
					"Option selected from High School Name Dropdown:",
					option
				);
		...
		vm.earlySchool.subscribe(function (option) {
			vm.earlySchoolSelectionWorkflow(option);
			console.log("EARLY SCHOOL SELECTION: ", option);
		});
		...
		if (pkg.stepCode === "Start") {
			...
			$("#earlyCollegeStuChk").on("change", function () {
				...
				console.log("Early College student Checkbox: ", isChecked);
				if (isChecked) {
					$(".coursesSection").hide();
				} else {
					$(".coursesSection").show();
		...
		if (
			...
			makeAutoCompleteReadOnly("highSchoolName");
