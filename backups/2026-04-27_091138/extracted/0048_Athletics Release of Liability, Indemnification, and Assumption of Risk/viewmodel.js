define([
    ...
    */
	...
	// vm.isOverEighteen = ko.observable('');
	// vm.overEighteen = ko.computed(function() {
	//     return vm.isOverEighteen();
	...
	vm.setDefaults = function setDefaults(source, inputValues) {
		...
		//integration.all('Web_Ethos_Get_Persons_by_Colleague_ID').then(function (studentDOB) {
		//console.log("Web_Ethos_Get_Persons_by_Colleague_ID", studentDOB);
		...
		if (user.isInLibrary) {
			...
			//vm.dateOfBirth('1/3/2007');

			console.log("source");
			let ethosSource = source.Web_Ethos_Get_Persons_by_Security_s_ERP_ID;

			if (ethosSource && ethosSource[0].dateOfBirth) {
				checkAge(ethosSource[0].dateOfBirth);
			}
		} else {
	...
	function parseDate(dateString) {
		...
		// Return an invalid date if the format is not recognized
		return new Date("Invalid Date");
	}

	function checkAge(checkThisAge) {
		const dateOfBirthString = checkThisAge;
		...
		const isOverEighteen = age >= 18;
		vm.overEighteen(isOverEighteen ? "Yes" : "No");
		//console.log("overEighteen:", vm.overEighteen());
	}
