define([
    ...
    */
	...
	vm.setDefaults = function setDefaults(source, inputValues) {
		...
		//Clear out the checkboxes if the information update type is changed
		vm.updateInformation.subscribe(function (newValue) {
			vm.homeAddressUpdateCheck(false);
			vm.emailAddressCheck(false);
			vm.personalPhoneNumberCheck(false);
			vm.emergencyContactCheck(false);
		});
			...
			integration
				...
				.then(function (employeeData) {
					...
					if (employeeData.names[0].middleName) {
						vm.employeeMiddleInitial(
							employeeData.names[0].middleName.substring(0, 1) +
								"."
						);
					} else {
					...
					integration
						...
						.then((institutionJobs) => {
							...
							//get the pay class to determine full-time part-time hourly etc.
							integration
								.first(
									"Web_Ethos_Get_Pay_Class_by_Pay_Class_GUID",
									{
										payClassGUID:
											institutionJobs.payClass.id
									}
								)
								.then((payClass) => {
									console.log("Pay Class: ", payClass);
									vm.payClassCode(payClass.code);
								});
							...
							integration
								...
								.then((position) => {
									...
									// integration.first('Web_Ethos_Get_Pay_Classifications_by_Classification_ID', {
									//     classificationGUID: position.classification.id
									// }).then((classification) => {
	...
	vm.afterRequired = function afterRequired(form) {
		...
		if (vm.homeAddressUpdateCheck()) {
			if (form.attachmentCount < 1) {
				notify(
					"error",
					"You must attach proof of address before submitting this form."
				);
				return false;
			}
		} else if (vm.updateInformation() == "Legal Name Change") {
			if (form.attachmentCount < 1) {
				notify(
					"error",
					"You must attach a copy of your social security card before submitting this form."
				);
				return false;
			}
		} else {
