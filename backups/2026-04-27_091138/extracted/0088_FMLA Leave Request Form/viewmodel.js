define([
    ...
    */
	...
	vm.setDefaults = function setDefaults(source, inputValues) {
		...
		vm.hrSignature.subscribe(function (newValue) {
			if (newValue) {
				vm.hrSignatureDate(Extenders.utils.formatDate(new Date()));
			} else {
				vm.hrSignatureDate("");
			}
		});

		vm.supSignature.subscribe(function (newValue) {
			if (newValue) {
				vm.supSignatureDate(Extenders.utils.formatDate(new Date()));
			} else {
				vm.supSignatureDate("");
			}
		});
		...
		if (user.isInLibrary) {
			...
			vm.hrDocType("FMLA Leave Request Form");
