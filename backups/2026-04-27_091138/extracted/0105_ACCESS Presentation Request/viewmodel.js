define([
    ...
    */
	...
	vm.setDefaults = function setDefaults(source, inputValues) {
		...
		vm.requesterSign.subscribe(function (newValue) {
			if (newValue) {
				vm.requesterSignDate(Extenders.utils.formatDate(new Date()));
			} else {
				vm.requesterSignDate("");
			}
		});
	...
	vm.afterLoad = function afterLoad() {
		...
		ko.computed(function () {
			if (user.isInLibrary || user.isInDrafts) {
				//calculate date two weeks from current date (this date is being used to set the start date with extenders on the form data fields)
				const parts = vm.originationDate().split("/");
				const month = parseInt(parts[0], 10) - 1; // JS months are 0-indexed
				const day = parseInt(parts[1], 10);
				const year = parseInt(parts[2], 10);

				const originalDate = new Date(year, month, day);
				originalDate.setDate(originalDate.getDate() + 14); // Add 14 days

				const newMonth = String(originalDate.getMonth() + 1).padStart(
					2,
					"0"
				);
				...
				vm.twoWeeksLater(`${newMonth}/${newDay}/${newYear}`);

				// //calculate academic year from current date for form import
				// const startDate = new Date(vm.originationDate());
				// const startYear = startDate.getFullYear();
				// const startMonth = startDate.getMonth();

				// let schoolYear;
				// if (startMonth <= 6) {
				//     schoolYear = startYear + "-" + (startYear + 1).toString().slice(-2);
				// } else {
				//     schoolYear = (startYear + 1) + "-" + (startYear + 2).toString().slice(-2);
				// }

				// vm.academicYear(schoolYear);
			}
		});
