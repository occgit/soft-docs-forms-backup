    ...
    */
	...
	vm.setDefaults = function setDefaults(source, inputValues) {
		...
		if (pkg.isAtStep("ConditionalExecutiveCouncilMember")) {
			...
			console.log(
				"Exec Council Display Name: ",
				vm.execCouncilDisplayName()
			);
		}
		...
		Promise.all(p_FormDraftCreation)
			.then((draftResults) => {
				draftResults.forEach((draftResponse, index) => {
					...
					console.log(
						`Create Form Draft API Response ${index + 1}:`,
						draftResponse
					);
				...
				Promise.all(p_FormDataUpdate)
					.then((dataResults) => {
						dataResults.forEach((dataResponse, dataIndex) => {
							// Handle successful API response
							console.log(
								`Create Form Data API Response ${dataIndex + 1}:`,
								dataResponse
							);
						});
						...
						Promise.all(p_FormSubmit)
							.then((submitResults) => {
								submitResults.forEach(
									(submitResponse, submitIndex) => {
										// Handle successful API response
										console.log(
											`Submit Form API Response ${submitIndex + 1}:`,
											submitResponse
										);
									}
								);
