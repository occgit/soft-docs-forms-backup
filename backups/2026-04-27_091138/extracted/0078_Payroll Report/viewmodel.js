	...
	vm.afterLoad = function afterLoad() {
		...
		ko.computed(function () {
			vm.weekList().forEach((dynamicListRow) => {
				...
				if (
					totalHours > 8 &&
					!["PT Hourly", "Student", "Work Study"].includes(
						vm.primaryStatus()
					)
				) {
	...
	function runEmployeeEthosCall(employeeID) {
		...
		integration
			...
			.then(function (personResults) {
				...
				return integration
					...
					.catch(function (error) {
						console.log("ACTIVE JOB LOOKUP ERROR:", error);
						return null;
					});
			})
			.then(function (allInstJob) {
				...
				console.log("ALL ACTIVE JOBS:", allInstJob);
				...
				const jobPromises = allInstJob.map(function (job) {
					...
					const positionGUID =
						job && job.position && job.position.id
							? job.position.id
							: "";
			...
			.then(function (results) {
				...
				console.log("POSITION/CAMPUS RESULTS:", results);
					...
					vm.personsPositionsList.push({
						...
						positionTitle:
							personPosition && personPosition.title
								...
								: "",
						positionCode:
							personPosition && personPosition.code
								? personPosition.code
								: "",
						campusTitle:
				...
				const primarySupervisorGUID =
					primaryJobResult.supervisorGUID || "";
				...
				return integration
					...
					.then(function (supervisorResult) {
						console.log(
							"PRIMARY SUPERVISOR RESULT:",
							supervisorResult
						);

						if (supervisorResult) {
							vm.supervisorName(
								getPreferredName(supervisorResult.names) || ""
							);
							vm.supervisorEmail(
