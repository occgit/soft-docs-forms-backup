    ...
    */
	...
	vm.setDefaults = function setDefaults(source, inputValues) {
		...
		if (user.isInLibrary || user.isInDrafts) {
			...
			vm.employeeStudent.subscribe(function (newValue) {
				$(".students").hide();
				$(".employees").hide();
				if (newValue) {
					if (newValue == "Student") {
			...
			integration
				...
				.then(function (employeeData) {
					...
					var partsofdate = employeeData.dateOfBirth.split("-");
					var formattedDate = `${partsofdate[1]}/${partsofdate[2]}/${partsofdate[0]}`;
					vm.employeeDateOfBirth(formattedDate);
					vm.employeeLastNameLegal(employeeData.names[0].lastName);
					vm.employeeFirstNameLegal(employeeData.names[0].firstName);
					...
					vm.phoneNumber(employeeData.phones[0].number);
					integration
						.first(
						...
						.then((institutionJobs) => {
							console.log("Institution Jobs: ", institutionJobs);
							integration
								.first(
