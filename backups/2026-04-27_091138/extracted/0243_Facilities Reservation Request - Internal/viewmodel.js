define(['jquery',
    ...
    */
    ...
    function makeAllFieldsReadOnly() {
        $("#roomLayout").prop("readonly", true);
        $("#describeOther").prop("readonly", true);


        $("#requestorSignature").prop("readonly", true);
        $("#requesterPhone").prop("readonly", true);

        $("#notes").prop("readonly", true);

        // //dropdown and checkboxes
        $('#employeeCampus').prop('disabled', true);
        $('#chkPodium').prop('disabled', true);
        $("#chkMicrophone").prop("disabled", true);
        $("#chkOther").prop("disabled", true);

        vm.addReservationRow().forEach(function (dynamicListRow, index) {
            dynamicListRow.campus.disabled = true;
            dynamicListRow.eventDate.readOnly = true;
            makeAutoCompleteReadOnly(dynamicListRow.startTime);
            makeAutoCompleteReadOnly(dynamicListRow.endTime);
            makeAutoCompleteReadOnly(dynamicListRow.arrivalTime);
            dynamicListRow.nameOfEvent.readOnly = true;
            dynamicListRow.roomPreferred.readOnly = true;
            dynamicListRow.expectedAttendance.readOnly = true;
            dynamicListRow.foodPlanned.disabled = true;
            dynamicListRow.partyDescription.readOnly = true;

            $(".btn-delete-item").prop("disabled", true);
        });

        $(".addReservationBtn").prop('disabled', true).attr('aria-disabled', 'true');
        $(".removeReservationBtn").prop('disabled', true).attr('aria-disabled', 'true');
    }
    ...
    vm.setDefaults = function setDefaults(source, inputValues) {
        ...
        vm.requestorSignature.subscribe(function (sig) {
            vm.requestorSignDate(getTodayDate());
        });
