define([
    ...
    */
	...
	vm.afterLoad = function afterLoad() {
        ...
        var doYouHaveInsurance = document.getElementById("doYouHaveInsurance").value;
        var attachmentCount = form.attachmentCount; // Assuming form.attachmentCount is defined elsewhere
        var requiredAttachments = 1; // Replace 'x' with the required number of attachments

         if (doYouHaveInsurance === "Yes") {
         if (attachmentCount < requiredAttachments) {
            notify('error', 'You must attach ' + requiredAttachments + ' document(s)');
            return false;
        }
    }
    return true;
		...
		ko.computed(function () {
			vm.studentName(vm.originatorDisplayName());
		});
		...
		ko.computed(function () {
			vm.studentAthleteSport(vm.studentSport());
		});
	...
	vm.afterRequired = function afterRequired(form) {
		...
		if (document.getElementById("doYouHaveInsurance").value === "Yes") {
			if (form.attachmentCount < 1) {
				notify("error", "You must attach Insurance Information");
				return false;
			}
		}
		return true;
