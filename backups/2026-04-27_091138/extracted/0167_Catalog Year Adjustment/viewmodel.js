    ...
    */
	...
	vm.afterLoad = function afterLoad() {
		...
		ko.computed(() => {
			if (vm.programTitle()) {
				vm.programSelected(
					vm.programTitle.element.selectedOptions[0].text
				);
			}
		});
						...
						integration
							...
							.then(function (programs) {
								...
								if (!programs || !programs.length) {
									notify(
										"error",
										"No programs found for this student."
									);
									if (dd)
										dd.innerHTML =
								...
								activePrograms.forEach(function (ap) {
									var guid =
										ap && ap.program && ap.program.id;
									var catalogId =
										ap && ap.catalog && ap.catalog.id;

									if (guid) {
										activeProgramGUIDs.push(guid);
										if (catalogId)
											programGUIDToCatalogGUID[guid] =
												catalogId;
								...
								// 3) Helper: Load Catalog Year (Calendar Year) for a given Program GUID
								function loadCatalogYearForProgramGUID(
									programGUID
								) {
									var catalogGUID =
										programGUIDToCatalogGUID[programGUID];

									if (!catalogGUID) {
										console.warn(
											"No catalog GUID found for program:",
											programGUID
										);
										if (typeof vm !== "undefined") {
											vm.studentCatalogYear("");
									...
									function formatCatalogYear(code) {
										...
										var m =
											String(code).match(
												/\b(19|20)\d{2}\b/
											); // restrict to 1900–2099; adjust if needed
										if (!m) return "UNK";
								...
								Promise.all(detailPromises)
									.then(function (detailsArray) {
										if (!dd) {
											console.warn(
												"Dropdown element #programTitle not found."
											);
											return;
										}
										...
										dd.onchange = function () {
											...
											// Update the title in vm if we can match by index
											var selIndex =
												activeProgramGUIDs.indexOf(
													selGUID
												);
											if (selIndex > -1) {
												var det =
													detailsArray[selIndex];
												if (
													typeof vm !== "undefined" &&
													det &&
													det.title
									...
									.catch(function (err) {
										...
										notify(
											"error",
											"Failed to load academic program details."
										);
										if (dd)
											dd.innerHTML =
							...
							.catch(function (err) {
								console.error(
									"Error fetching student programs:",
									err
								);
								notify(
									"error",
									"Failed to load student programs."
								);
								var ddErrRoot =
									document.getElementById("programTitle");
								if (ddErrRoot)
