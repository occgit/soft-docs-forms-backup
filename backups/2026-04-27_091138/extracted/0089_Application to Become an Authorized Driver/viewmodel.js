define([
    ...
    */
	...
	vm.setDefaults = function setDefaults(source, inputValues) {
		...
		//Making employee section readonly if not at start step
		if (!pkg.isAtStep("Start")) {
			$(
				"#employeeDriversLicenseNumber,#supervisorEmail, #reasonForVehicleUse, #employeeSignature"
			).prop("readonly", true);
			makeAutoCompleteReadOnly("stateOfIssue");
			makeAutoCompleteReadOnly("supervisorSearch");
		}

		//making supervisor signature readonly if not at supervisor step
		if (!pkg.isAtStep("ConditionalActorRouteSupervisorEmail")) {
			//
			$("#supervisorSignature").prop("readonly", true);
		}

		vm.supervisorEmail.subscribe(function (email) {
			vm.routeSupervisorEmail(email);
		});

		//Hiding and showing HR Section/Supervisor Section depending on the workflow step
		if (pkg.isAtStep("WFHRManualEntry") || pkg.isAtStep("End")) {
			$(".humanResources").show();
		}

		if (
			pkg.isAtStep("ConditionalActorRouteSupervisorEmail") ||
			pkg.isAtStep("WFHRPublicSafety") ||
		...
		if (user.isInLibrary) {
			...
			vm.hrDocType("Application to Become Authorized Driver");
						...
						.then((institutionJobs) => {
							...
							integration
								.all("Web_Ethos_Get_Person_by_GUID", {
									personGUID: getPrimarySupervisor(
										institutionJobs.supervisors
									)
								})
	...
	vm.afterLoad = function afterLoad() {
		...
		ko.computed(function () {
			if (vm.employeeLastName() && vm.employeeFirstName()) {
				vm.employeeName(
					vm.employeeFirstName() + " " + vm.employeeLastName()
				);
			} else {
				vm.employeeName("");
			}
