...
define(['jquery',
    ...
    */
    ...
    vm.setDefaults = function setDefaults(source, inputValues) {
        ...
        // if (pkg.isAtStep('ConditionalActorGetAdministrativeSpecialist') || pkg.isAtStep('End')) {
        //     $('.appComments').show();
        // }

        if (!pkg.isAtStep('Start') && !pkg.isAtStep('ConditionalActorEmployee')) {
            $('.appComments').show();
        }
        ...
        // vm.academicPeriodGuid.subscribe(function (newValue) {
        //     showSpinner();
        ...
        //             let sectionName = SectionRow.code;
        //                 vm.SectionsAutocomplete.push({
        //                     secName: SectionRow.code,
        //                     secStatus: SectionRow.status,
        //                     secWeeks: SectionRow.durationLength + ' ' + SectionRow.durationUnit,
        //                     secCapacity: SectionRow.maxEnrollment,
        //                     secCrossListed: SectionRow.crossListed,
        //                     secStartDate: SectionRow.startOn,
        //                     secEndDate: SectionRow.endOn,
        //                     secMeetingTimes: SectionRow.meetingTimes,
        //                     secTerm: SectionRow.academicPeriod16Code,
        //                     secSynonym: SectionRow.secSynonym
        //                 });
        //             //}
        //         });
        ...
        ko.computed(function () {
            ...
            integration.all('Web_Custom_Sections', { termGUID: term })
                .then(function (WebSections) {
                    ...
                    function getCampusGroup(letter) {
                        ...
                        if (["A", "D", "X", "Y", "Z"].includes(letter)) return "AUBURN";
        ...
        // integration.all('Web_Ethos_Get_Buildings',{
        ...
        vm.addBuildingGUID.subscribe(function (newValue) {
            showSpinner();
            integration.all('Web_Ethos_Get_Rooms_filtered_by_Building', {
        ...
        vm.newBuildingGUID.subscribe(function (newValue) {
            ...
            integration.all('Web_Ethos_Get_Rooms_filtered_by_Building', {
                ...
                console.log('Failed to Filtered Room by Building List.', err);
            }).finally(hideSpinner);
        ...
        vm.addSubjectID.subscribe(function (newVal) {
            ...
            if (newVal) {
                showSpinner();
                getAllDataFromPagingSource('Web_Sources_Get_Courses_by_SubjectGUID', newVal, 100).then(listOfCoursesBySubjectData => {

                    // loop through the results
                    listOfCoursesBySubjectData.forEach(function (listOfCoursesBySubjectDataRow) {
                        if (listOfCoursesBySubjectDataRow.status.id === '73c76722-884e-43e9-b093-8ebe092c045f' && listOfCoursesBySubjectDataRow.credits[0].creditCategory.creditType === 'institution') {
                            // push custom attributes into autocomplete
                            vm.CourseSubjectAutocomplete.push({
                                courseTitle: listOfCoursesBySubjectDataRow.titles[0].value,
                                courseNumber: listOfCoursesBySubjectDataRow.number,
                                courseSubjectAndNumber: vm.addSubject() + ' ' + listOfCoursesBySubjectDataRow.number
                            });
                        }
                    });
        ...
        // ko.computed(function () {
        //     const selectedSubj = vm.selectedSubject();
        //     const campus = vm.campus();

        //     if (selectedSubj && campus) {
        //         console.log("Selected Subject:", selectedSubj);
        //         console.log("Selected Campus:", campus);

        //         integration.first('Get_Academic_Routing_Data', {
        //             subject: selectedSubj,
        //             campus: campus
        //         }).then(function (RoutingData) {
        //             console.log("ROUTING DATA", RoutingData);
        //             vm.deanEmail(RoutingData.Dean_Email);
        //             vm.departmentChairEmail(RoutingData.Department_Chair_Email);
        //             vm.administrativeSpecialistEmail(RoutingData.Administrative_Specialist_Email);
        //         });
        //     }
        // });
        ...
        //             if (vm.deanEmail().toUpperCase() == vm.originatorEmail().toUpperCase()) { 
        // 	            vm.skipDean('Yes');
        //             }
        //             else{
        //                 vm.skipDean('No');
        ...
        //             if (vm.departmentChairEmail().toUpperCase() == vm.originatorEmail().toUpperCase()) { 
        ...
        vm.addInstructionalMethod.subscribe(function (newValue) {
            if (newValue === 'Hybrid' || newValue === 'In-Person' || newValue === 'Online Course with in-person Testing') {
                vm.addStartTime.required = true;
                vm.addEndTime.required = true;
                vm.addRoom.required = true;
            } else if (newValue === 'Flexible Live' || newValue === 'Flexible Online' || newValue === 'Online Live Course') {
                vm.addStartTime.required = true;
                vm.addEndTime.required = true;
                vm.addRoom.required = false;
            } else {
                vm.addStartTime.required = false;
                vm.addEndTime.required = false;
                vm.addRoom.required = false;
            }

            if (utilityFunctions) {
                utilityFunctions.sync('addStartTime');
                utilityFunctions.sync('addEndTime');
                utilityFunctions.sync('addRoom');
            }
        });
        ...
        //         vm.newStartTime.required = true;
        //         vm.newEndTime.required = true;
        //       //  vm.newRoom.required = true;
        //     } else if(newValue === 'Flexible Live' || newValue === 'Flexible Online' || newValue === 'Online Live Course'){
        //         vm.newStartTime.required = true;
        //         vm.newEndTime.required = true;
        ...
        //         vm.newStartTime.required = false;
        //         vm.newEndTime.required = false;
        //       // vm.addRoom.required = false;
        //     }
        // });
    ...
    vm.afterLoad = function afterLoad() {
        ...
        ko.computed(() => {
            ...
            if (vm.selfOrElse()) {
                if (vm.selfOrElse() === 'Myself') {
                    ...
                    if (vm.deanEmail()) {
                        if (vm.deanEmail().toUpperCase() == vm.employeeEmail().toUpperCase()) {
                            vm.skipDean('Yes');
                        } else {
                            vm.skipDean('No');
                        }
                    ...
                    if (vm.departmentChairEmail()) {
                        if (vm.departmentChairEmail().toUpperCase() == vm.employeeEmail().toUpperCase()) {
                            vm.skipDepartmentChair('Yes');
                        } else {
                            vm.skipDepartmentChair('No');
                        }
        ...
        ko.computed(() => {
            ...
            if ((user.isInLibrary || user.isInDrafts) && (vm.selectedTerm() && vm.action() === 'Change a Section')) {
                ...
                if (vm.selectedTerm().endsWith('/AY')) {
                    ...
                    showSpinner();
                    integration.first('FS_Get_Active_Term_Date_by_Term_Code', {
                        ...
                        var termDateObj = new Date(parts[2], parts[0] - 1, parts[1]); // year, month (0-based), day
                        ...
                        if (today >= termDateObj) {
                            ...
                            notify('error', 'Section changes can no longer be made for this term, as it is already active. To make an update, please cancel the original section and add a new one.');
                            $('.changeSectionRow').hide();
                        } else {
                            vm.allowSectionChange('Yes');
                            $('.changeSectionRow').show();
    ...
    vm.onDecline = function onDecline(form) {
        ...
        if (pkg.isAtStep('ConditionalActorGetDean') || pkg.isAtStep('ConditionalActorDepartmentChair')) {
            if (vm.reasonForDecline()) {
                return true;
            }
            else {
                $('#reasonForDecline').prop('required', true);
                throw new Error('Please enter a reason for declining at the bottom of the form and click Decline again to move the form along in the workflow.');
            }
        }
    ...
    function sectionExistsInAutocomplete(secName) {
        ...
        return list.some(r => normalizeSectionName(r && r.secName) === target);
    }
    ...
    vm.beforeRequired = function beforeRequired(form) {
        ...
        if (action === 'Change a Section') {
            ...
            requireValidSection(vm.changeSecName && vm.changeSecName(), 'Section Name (Change)');
        }

        if (action === 'Cancel a Section') {
            // cancelSecName is required in HTML, but this makes it "must match dataset"
            requireValidSection(vm.cancelSecName && vm.cancelSecName(), 'Section Name (Cancel)');
        }
    ...
    vm.afterRequired = function afterRequired(form) {
        ...
        // if (vm.allowSectionChange() == 'No') {
        //     throw new Error ('Section changes can no longer be made for this term, as it is already active. To make an update, please cancel the original section and add a new one.');
        // } else {
        //     return true;
        // }
        ...
        if (pkg.isAtStep('Start') && (instructionalMethodAddCourse === 'In-Person' || instructionalMethodAddCourse === 'Online Live Course' || instructionalMethodAddCourse === 'Flexible Online' || instructionalMethodAddCourse === 'Flexible Live')) {
        ...
        //ensuring at least one checkbox from meeting day(s) is checked (when changing a course)
        //  let instructionalMethodChangeCourse = vm.newInstructionalMethod();
        //  if(pkg.isAtStep('Start') && (instructionalMethodChangeCourse === 'Online Live Course' || instructionalMethodChangeCourse === 'Flexible Online' || instructionalMethodChangeCourse=== 'Flexible Live') ){
        //          const checkboxes = $('input[type="checkbox"][id^="new"]');
        //          const oneChecked = checkboxes.is(':checked');
        //          if (!oneChecked) {
        //           notify('warning', 'Please select at least one Meeting Day!');
        //           throw Error;
    ...
    function actionRowShow(actionValue) {
        switch (actionValue) {
            case 'Add a Section':
                ...
                if (pkg.isAtStep('ConditionalActorGetAdministrativeSpecialist') || pkg.isAtStep('End')) {
                    $('.addSectionCol').show();
                    $('#sectionNumber, #addSynonym').prop('required', true);
                }
                break;
