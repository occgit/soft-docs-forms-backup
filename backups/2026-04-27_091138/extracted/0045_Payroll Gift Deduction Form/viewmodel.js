		...
		integration
			...
			.then((employeeInfo) => {
				if (employeeInfo) {
					...
					const schoolEmail = employeeInfo.emails.find(
						(email) => email.type.emailType === "school"
					);
					const otherEmail = employeeInfo.emails.find(
						(email) => email.type.emailType === "other"
					);
					const schoolEmailAddress = schoolEmail
						? schoolEmail.address
						: null;
					const otherEmailAddress = otherEmail
						? otherEmail.address
						: null;
					...
					const primaryAddress = employeeInfo.addresses.find(
					...
					notify(
						"error",
						"No employee information found. Please try again with another ID Number. "
					);
				}
			});
	});
    ...
    */
	...
	vm.afterRequired = function afterRequired(form) {
		...
		if (vm.newDeductionAmount() < 10 && vm.increaseCurrentAmount() < 10) {
			throw new Error(
				"The amount of your Payroll Deduction must be at least $10.00 to submit this form."
			);
		}
	};
