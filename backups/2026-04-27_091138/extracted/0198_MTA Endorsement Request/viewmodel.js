define(['jquery',
    ...
    */
    ...
    vm.setDefaults = function setDefaults(source, inputValues) {
        ...
        if (user.isInLibrary) {
            ...
            integration.first('Web_Ethos_Get_Persons_by_Colleague_ID', { personID: vm.originatorErpId() }).then(function (personResults) {
                ...
                if (personResults) {
                    ...
                    const studentEmail = personResults.emails.find(email => email.preference === "primary");
                    const studentMiddleName = personResults.names.find(name => name.preference === "preferred");

                    //format date mm/dd/yyyy instead of yyyy-mm-dd
                    const [year, month, day] = personResults.dateOfBirth.split("-");
                    const studentDOB = `${month}/${day}/${year}`;

                    vm.DOB(studentDOB);
                    vm.studentEmail(studentEmail.address);
                    vm.MiddleName(studentMiddleName.middleName);
                } else {
        ...
        //
        //Populating today's date on signature
        vm.ProcessedBy.subscribe(function (sig) {
            vm.ProcessedDate(getTodayDate());
        });

        if (pkg.isAtStep('Registrar')) {
            document.getElementById("ProcessedBy").disabled = false;
            document.getElementById("ReasonForDecline").disabled = false;
            document.getElementById('StudentSignature').disabled = true; //disable student signature
        }
    ...
    vm.onApprove = function onApprove(form) {
        ...
        if (pkg.isAtStep('Registrar')) {
            requiredIndicators.setRequired('ProcessedBy', true);
        }
    };
    ...
    vm.onDecline = function onDecline(form) {
        ...
        $('.OnDecline').show();
        requiredIndicators.setRequired('ReasonForDecline', trueOrFalse);
